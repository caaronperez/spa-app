...
enum Sections: Int {
    ...
    static let mapper: [Sections: String] = [
        .one: "Swedish Massage",
        .two: "Deep Tissue Massage",
        .three: "Sport Massage",
        .four: "Reflexology",
        .five: "Trigger Point Therapy"
    ]
    
    var string: String {
        return Sections.mapper[self]!
    }
    
    static let count: Int = {
        var max: Int = 0
        while let _ = Sections(rawValue: max) { max += 1 }
        return max
    }()
}
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        tableView.layer.cornerRadius = Corners.large.rawValue
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let value = Sections(rawValue: indexPath.row) {
    ...
    fileprivate func adjustScrollView() {
        scrollView.contentSize =
            CGSize(width: scrollView.frame.width * CGFloat(numPages),
                   height: scrollView.frame.height - view.safeAreaInsets.top * 2)
    }
    ...
    fileprivate func loadPage(_ page: Int) {
        ...
        if pages[page] == nil {
            if let image = UIImage(named: "\(page + 1)") {
                ...
                frame.origin.y = -view.safeAreaInsets.top * 2
                frame.size.height += view.safeAreaInsets.top * 2
